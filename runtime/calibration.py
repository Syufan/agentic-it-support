...
from runtime.constants import CONFIDENCE_LOW
...
def calibrate(raw: float, case: CaseState, retry_penalty: float) -> float:
    calibrated = raw

    if case.tool_calls_total == 0:
        calibrated = min(calibrated, _NO_EVIDENCE_CAP)

    calibrated -= retry_penalty * case.resolution_attempts
